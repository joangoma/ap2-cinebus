...
def get_osmnx_graph() -> OsmnxGraph:
    """Returns a graph of the streets of Barcelona. Get_only_first_edge should be tested"""
...
def add_weights(city: CityGraph) -> None:
    ...
    Primer es calculen els pesos dels carrers ja que es necessiten per calcular els pesos dels busos
    """
    ...
    for edge in city.edges(data=True):
        if edge[2]["type"] == "Carrer":
            ...
            city.edges[edge[0], edge[1]]["weight"] = (
                haversine(city.nodes[edge[0]]["coord"], city.nodes[edge[1]]["coord"])
                / WALK_SPEED
                * 60
            )
...
def build_city_graph(g1: OsmnxGraph, g2: BusesGraph) -> CityGraph:
    """If the citygraph is stored in FILE_CITY_NAME, it is loaded. Otherwise, g1 and g2 are merged to build a Citygraph
    and it is saved in FILE_CITY_NAME.
...
def find_path(ox_g: OsmnxGraph, g: CityGraph, src: Coord, dst: Coord) -> Path:
    """Returns a tuple with the list of nodes ids of the shortest path from src to dst and the minutes it takes."""
    """nota: shortest_path: If the source and target are both specified, return a single list of nodes in a shortest path from the source to the target.
    """
    #  ox.distance.nearest_nodes uses lon-lat coordinates, so we need to swap them
...
def plot(g: CityGraph, filename: str) -> None:
    """Saves the graph g as an image with the city map in the background in the file filename"""
...
def plot_path(g: CityGraph, p: Path, filename: str, *args) -> None:
    ...
    for i, id_node in enumerate(p[0]):
        ...
        map.add_marker(CircleMarker((node["coord"][1], node["coord"][0]), color, 3))
...
plot_path(
    city,
    find_path(
        osmx_g, city, (41.37949301243075, 2.121040855869357), (41.360067, 2.138944)
    ),
    "path_delay.png",
)
